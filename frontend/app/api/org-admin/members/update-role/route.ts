...
/**
 * POST /api/org-admin/members/update-role
 * Body: { org_id, user_id, role } (role: 1 = owner, 2 = admin, 3 = member). Requires org admin or owner.
 */
export async function POST(request: NextRequest) {
  ...
  let body: { org_id?: string; user_id?: string; role?: number };
  try {
  ...
  const role = body.role ?? orgAdmin.MembershipRole.MEMBER;
  if (!orgId || !userId) {
    return NextResponse.json({ error: "org_id and user_id required." }, { status: 400 });
  }
  try {
    const res = await orgAdmin.updateRole(token, orgId, userId, role);
