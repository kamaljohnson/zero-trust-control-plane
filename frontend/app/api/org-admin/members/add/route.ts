...
/**
 * POST /api/org-admin/members/add
 * Body: { org_id, user_id, role } (role: 2 = admin, 3 = member). Requires org admin or owner.
 */
export async function POST(request: NextRequest) {
  ...
  try {
    const res = await orgAdmin.addMember(token, orgId, userId, role);
